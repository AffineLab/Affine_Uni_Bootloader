...
extern void boot_stm32h503_usb_receive_isr(const uint8_t *data, uint32_t length);
...
static int8_t CDC_Receive(uint8_t cdc_ch, uint8_t *Buf, uint32_t *Len)
{
    boot_stm32h503_usb_diag_note_rx(*Len);
    boot_stm32h503_usb_receive_isr(Buf, *Len);
...
static int8_t CDC_TransmitCplt(uint8_t cdc_ch, uint8_t *Buf, uint32_t *Len, uint8_t epnum)
{
    ...
    (void)Buf;
    (void)epnum;
    boot_stm32h503_usb_diag_note_tx_complete((Len != NULL) ? *Len : 0U);
    return (int8_t)USBD_OK;
}
